        ...
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-product" class="form-horizontal">
          ...
          <div class="tab-content">
            ...
            <div class="tab-pane" id="tab-links">
              ...
              <div class="form-group"> <label class="col-sm-2 control-label" for="input-bundled"><span data-toggle="tooltip" title="<?php echo $help_related; ?>"><?php echo $entry_bundled; ?></span></label> <div class="col-sm-10"> Quantity: <input type="text" name="bundle_qty" value="1" id="bundle_qty" class="form-control" /> Product (autocomplete) : <input type="text" name="bundled" value="" placeholder="<?php echo $entry_bundled; ?>" id="input-bundled" class="form-control" /> <div id="product-bundled" class="well well-sm" style="height: 150px; overflow: auto;"> <?php $bundle_counter=0; foreach ($product_bundled as $product_bundle) { $bundle_counter++; ?> <div id="product-bundled<?php echo $product_bundle['product_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $product_bundle['qty']; ?> x <?php echo $product_bundle['name']; ?> <input type="hidden" name="product_bundled[<?php echo $bundle_counter; ?>][product_id]" value="<?php echo $product_bundle['product_id']; ?>" /> <input type="hidden" name="product_bundled[<?php echo $bundle_counter; ?>][qty]" value="<?php echo $product_bundle['qty']; ?>" /> </div> <?php } ?> </div> </div> </div>
...
var bundle_counter=<?php echo $bundle_counter; ?>; $('input[name=\'bundled\']').autocomplete({ 'source': function(request, response) { $.ajax({ url: 'index.php?route=catalog/product/autocomplete&token=<?php echo $token; ?>&filter_name=' + encodeURIComponent(request), dataType: 'json', success: function(json) { response($.map(json, function(item) { return { label: item['name'], value: item['product_id'] } })); } }); }, 'select': function(item) { bundle_counter++; $('input[name=\'bundled\']').val(''); $('#product-bundled' + item['value']).remove(); $('#product-bundled').append('<div id="product-bundled' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + $('#bundle_qty').val()+' x '+item['label'] + '<input type="hidden" name="product_bundled['+bundle_counter+'][product_id]" value="' + item['value'] + '" /><input type="hidden" name="product_bundled['+bundle_counter+'][qty]" value="' + $('#bundle_qty').val() + '" /></div>'); } }); $('#product-bundled').delegate('.fa-minus-circle', 'click', function() { $(this).parent().remove(); });
