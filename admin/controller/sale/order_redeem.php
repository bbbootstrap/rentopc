<?php
class ControllerSaleOrderredeem extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$this->load->model('sale/orderredeem');
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			'text' => "Redemption Center Orders",
			'href' => $this->url->link('sale/order_redeem', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$order_total = $this->model_sale_orderredeem->getTotalOrders();

		$results = $this->model_sale_orderredeem->getOrders();

		foreach ($results as $result) {
			$data['orders'][] = array(
				'order_id'      => $result['order_id'],
				'customer'      => $result['payment_firstname'],
				'history'       => $this->model_sale_orderredeem->getOrderHistories($result['order_id']),
				'status'        => $result['status'],
				'total'         => $this->currency->format($result['total'], $result['currency_code'], $result['currency_value']),
				'date_added'    => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
				'date_modified' => date($this->language->get('date_format_short'), strtotime($result['date_modified'])),
				'shipping_code' => $result['shipping_code'],
				'view'          => $this->url->link('sale/order_redeem/info', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL'),
				// 'edit'          => $this->url->link('sale/order/edit', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL'),
			);
		}

		$data['heading_title'] = "Redemption Center Orders";

		$data['text_list'] = "Redemption Center Orders";
		...
		$pagination->limit = 10;
		$pagination->url = $this->url->link('sale/order_redeem', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('sale/order_list_redeem.tpl', $data));
	}
	...
	public function info() {
		...
		$order_info = $this->model_sale_orderredeem->getOrder($order_id);
		...
		if ($order_info) {
			...
			$data['heading_title'] = "Redemption Center";
			...
			$products = $this->model_sale_orderredeem->getOrderProducts($this->request->get['order_id']);

			foreach ($products as $product) {
				$option_data = array();

				$options = $this->model_sale_orderredeem->getOrderOptions($this->request->get['order_id'], $product['order_product_id']);
				...
				$data['history'] = $this->model_sale_orderredeem->getOrderHistories($order_id);
				...
				$data['products'][] = array(
					...
					'option'   		   => $option_data,
					'quantity'		   => $product['quantity'],
					'price'    		   => round($product['points'],0) ,
					'total'    		   => round($product['total'],0),
			...
			$this->response->setOutput($this->load->view('sale/order_info_redeem.tpl', $data));
		} else {
	...
	public function history() {
		...
		$results = $this->model_sale_orderredeem->getOrderHistories($this->request->get['order_id'], ($page - 1) * 10, 10);
