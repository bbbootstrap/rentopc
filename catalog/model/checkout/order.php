...
class ModelCheckoutOrder extends Model {
	...
	public function addRedeem($data) {
		...
		//$this->db->query("INSERT INTO `" . DB_PREFIX . "order_redeem` SET customer_id = '" . (int)$data['customer_id'] . "', closing_redeem_points = '" . (int)$data['customer_id'] . "', date_created = NOW()");

		$this->db->query("INSERT INTO `" . DB_PREFIX . "order_redeem` SET customer_id = '" . (int)$data['customer_id'] . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', payment_firstname = '" . $this->db->escape($data['payment_firstname']) . "', payment_lastname = '" . $this->db->escape($data['payment_lastname']) . "', payment_company = '" . $this->db->escape($data['payment_company']) . "', payment_address_1 = '" . $this->db->escape($data['payment_address_1']) . "', payment_address_2 = '" . $this->db->escape($data['payment_address_2']) . "', payment_city = '" . $this->db->escape($data['payment_city']) . "', payment_postcode = '" . $this->db->escape($data['payment_postcode']) . "', payment_country = '" . $this->db->escape($data['payment_country']) . "', payment_country_id = '" . (int)$data['payment_country_id'] . "', payment_zone = '" . $this->db->escape($data['payment_zone']) . "', payment_zone_id = '" . (int)$data['payment_zone_id'] . "', payment_address_format = '" . $this->db->escape($data['payment_address_format']) . "', payment_custom_field = '" . $this->db->escape(isset($data['payment_custom_field']) ? json_encode($data['payment_custom_field']) : '') . "', payment_method = '" . $this->db->escape($data['payment_method']) . "', payment_code = '" . $this->db->escape($data['payment_code']) . "', shipping_firstname = '" . $this->db->escape($data['shipping_firstname']) . "', shipping_lastname = '" . $this->db->escape($data['shipping_lastname']) . "', shipping_company = '" . $this->db->escape($data['shipping_company']) . "', shipping_address_1 = '" . $this->db->escape($data['shipping_address_1']) . "', shipping_address_2 = '" . $this->db->escape($data['shipping_address_2']) . "', shipping_city = '" . $this->db->escape($data['shipping_city']) . "', shipping_postcode = '" . $this->db->escape($data['shipping_postcode']) . "', shipping_country = '" . $this->db->escape($data['shipping_country']) . "', shipping_country_id = '" . (int)$data['shipping_country_id'] . "', shipping_zone = '" . $this->db->escape($data['shipping_zone']) . "', shipping_zone_id = '" . (int)$data['shipping_zone_id'] . "', shipping_address_format = '" . $this->db->escape($data['shipping_address_format']) . "', shipping_custom_field = '" . $this->db->escape(isset($data['shipping_custom_field']) ? json_encode($data['shipping_custom_field']) : '') . "', shipping_method = '" . $this->db->escape($data['shipping_method']) . "', shipping_code = '" . $this->db->escape($data['shipping_code']) . "', comment = '" . $this->db->escape($data['comment']) . "', total = '" . (float)$data['total'] . "', affiliate_id = '" . (int)$data['affiliate_id'] . "', commission = '" . (float)$data['commission'] . "', marketing_id = '" . (int)$data['marketing_id'] . "', tracking = '" . $this->db->escape($data['tracking']) . "', language_id = '" . (int)$data['language_id'] . "', currency_id = '" . (int)$data['currency_id'] . "', currency_code = '" . $this->db->escape($data['currency_code']) . "', currency_value = '" . (float)$data['currency_value'] . "', ip = '" . $this->db->escape($data['ip']) . "', forwarded_ip = '" .  $this->db->escape($data['forwarded_ip']) . "', user_agent = '" . $this->db->escape($data['user_agent']) . "', accept_language = '" . $this->db->escape($data['accept_language']) . "', date_added = NOW(), date_modified = NOW()");
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product_redeem SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', points = '" . (float)$product['points'] . "', total = '" . (float)$product['total'] . "'");
	...
	public function getOrder_redeem($order_id) {
		//$order_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order` o WHERE o.order_id = '" . (int)$order_id . "'");
		$order_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_redeem` WHERE order_id = '" .  (int)$order_id  . "'");
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
                ...
                if ($this->config->get('synicsys_status') == "1") {
                    ...
                    $hello ="";
                    $message = "Dear ".$order_info['firstname']." , Your order has been placed. ";
                    ...
                    if (isset($order_info['telephone'])) {
                        $to = $order_info['telephone'];
                        $simba = $sms -> send($to,$message,$api_key,$sender_id,$api_url);
                    }

                   // $simba = $sms -> send(9896925503,$message,$api_key,$sender_id,$api_url);
				...
				if ($this->config->get('synicsys_status') == "1") {
					...
					$admin_phone = $this->config->get('config_telephone');

					$owner_name = $this->config->get('config_owner');
					$message = "Hello ".$owner_name.", you received new order with order ID ".$this->session->data['order_id']."";
					$sms -> send($admin_phone,$message,$api_key,$sender_id,$api_url);
...
public function addRedemptionPoints($customer_id,$reward,$comment){

	$this->db->query("INSERT INTO " . DB_PREFIX . "customer_reward SET customer_id = '" . (int)$customer_id . "', order_id = 0, points = '" . -(int)$reward . "', description = '" . $this->db->escape($comment) . "', date_added = NOW()");
	...
	public function addOrderHistoryapi($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		$event_data = array(
			'order_id'		  => $order_id,
			'order_status_id' => $order_status_id,
			'comment'		  => $comment,
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				if ($this->config->get('synicsys_status') == "1") {
					...
					$owner_name = $this->config->get('config_owner');
					$message = "Hello ".$owner_name.", A existing order($order_id) has been updated successfully";
					$sms -> send($admin_phone,$message,$api_key,$sender_id,$api_url);
	...
	public function addRedeemOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		$event_data = array(
			'order_id'		  => $order_id,
			'order_status_id' => $order_status_id,
			'comment'		  => $comment,
		...
		$order_info = $this->getOrder_redeem($order_id);
		...
		if ($order_info) {
			...
			// Update the DB with the new statuses
			$this->db->query("UPDATE `" . DB_PREFIX . "order_redeem` SET order_status_id = '" . (int)$order_status_id . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");

			$this->db->query("INSERT INTO " . DB_PREFIX . "redeem_order_history SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$order_status_id . "', notify = '" . (int)$notify . "', comment = '" . $this->db->escape($comment) . "', date_added = NOW()");
...
// Admin Alert Mail
					...
					$owner_name = $this->config->get('config_owner');
					$message = "Hello ".$owner_name.", you received new redemption order with order ID ".$this->session->data['order_id']."";
					$sms -> send($admin_phone,$message,$api_key,$sender_id,$api_url);
