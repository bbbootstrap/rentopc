...
class ModelCatalogProduct extends Model {
  ...
  //
  //   $bundle_products=$this->db->query("Select bundled_id,qty FROM " . DB_PREFIX ."product_bundle where product_id=".$product_id."");
  //
  // }
  ...
  public function getProductslivesearch($data = array()) {
	...
	public function getProductFaqs($product_id) {
		$product_faqs_data = array();
		$product_faqs_query = $this->db->query("SELECT * FROM oc_product_faqs WHERE product_id ='" . (int)$product_id . "'");

		foreach ($product_faqs_query->rows as $product_faqs) {
				$product_faqs_data[] = array(
					'product_id' => $product_faqs['product_id'],
					'faq_question'         => $product_faqs['faq_question'],
					'faq_answer'         => $product_faqs['faq_answer']
				);
			}

			return $product_faqs_data;
    ...
    public function getProductReviews($product_id) {
		...
		$product_review_query = $this->db->query("SELECT * FROM oc_review WHERE product_id ='" . (int)$product_id . "'");

		foreach ($product_review_query->rows as $product_review) {
				$product_review_data[] = array(
  ...
  public function getProductRewards($product_id){
    ...
    $query=$this->db->query("SELECT points FROM " . DB_PREFIX . "product_reward WHERE product_id =".$product_id." AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "'");
    if ($query->num_rows) {
  ...
  public function getcomboProduct2($product_id){
    $combo_product_data = array();
    $query=$this->db->query("Select bundled_id,qty FROM " . DB_PREFIX ."product_bundle_2 where product_id=".$product_id."");
    foreach ($query->rows as $result) {
      $combo_product_data[$result['bundled_id']] = $this->getProduct($result['bundled_id']);
  ...
  public function getcomboProductQty($product_id,$bundled_id){

    $query=$this->db->query("Select qty FROM " . DB_PREFIX ."product_bundle where product_id=".$product_id." and bundled_id=".$bundled_id."");
  ...
  public function getcomboProductQty2($product_id,$bundled_id){

    $query=$this->db->query("Select qty FROM " . DB_PREFIX ."product_bundle_2 where product_id=".$product_id." and bundled_id=".$bundled_id."");
