		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($order_info) {
				...
				if (!$json) {
					...
					$this->model_checkout_order->addOrderHistoryapi($order_id, $order_status_id);
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($order_info) {
				...
				if($this->request->post['order_status_id'] == 7){
						...
						$comment = "Points deducted against cancelled Order #".$order_id;
						$this->model_checkout_order->addRedemptionPoints($customer_id,$reward,$comment);
