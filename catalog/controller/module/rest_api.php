...
class ControllerModuleRestApi extends Controller
{
	...
	private function getOrders()
	{
		...
		$orders = $this->model_module_rest_api->setFilters($this->filters)
														->getOrders();
		foreach ($orders as $key => $order) {

			$products = $this->model_module_rest_api->getOrderProducts($order['order_id']);
	...
	{
		$products = $this->model_module_rest_api->setFilters($this->filters)
														  ->getProducts();
	...
	private function getProduct()
	{
		if ($this->has('product_id')) {
			...
			$product = $this->model_module_rest_api->getProduct($this->get('product_id'));
	...
	public function updatePrice()
	{
		if (isset($this->filter->sku) && isset($this->request->get['price'])) {
			$this->model_module_rest_api->updateProductPrice($this->filter->sku, $this->request->get['price']);
	...
	public function changeProduct($quantity, $type)
	{
		if (isset($quantity) && isset($type)) {
			...
			$model = $this->model_module_rest_api;
	...
	private function getProductAttribute($products)
	{
		...
		foreach ($products as $product) {
			$product = $this->model_module_rest_api->getProduct($product['product_id']);
