...
class ControllerModuleBanner extends Controller {
	public function index($setting) {
		...
		// $this->document->addScript('catalog/view/javascript/jquery/owl-carousel/owl.carousel.min.js');
		 ...
		 $this->document->addStyle('https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/assets/owl.theme.default.css');
		 $this->document->addScript('https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.2.1/owl.carousel.js');
		...
		foreach ($results as $result) {
			if (is_file(DIR_IMAGE . $result['image'])) {
				$data['banners'][] = array(
					'title' => $result['title'],
					'link'  => $result['link'],
					 'image' => $this->model_tool_image->resize($result['image'],1384,280)
