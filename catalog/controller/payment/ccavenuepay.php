...
class ControllerPaymentCcavenuepay extends Controller {
	public function index() {
		...
		if ($order_info){
		   $currency =  $cbdom->getAllowedCurrency($order_info['currency_code']); 
			...
			//$data['action'] 	="https://test.ccavenue.com/transaction/transaction.do?command=initiateTransaction";
            ...
            $encrypted_data = $cbdom->encrypt($merchant_data,$workingKey);
			$data['api_resonse'] = $workingKey;

            $data['encrypted_data'] = $encrypted_data;
			
			
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/ccavenuepay.tpl',$data)) {
	...
	public function callback() 
	{
		...
		if($order_info)
		{
			...
			unset($this->session->data['cc_tracking_id']);
			unset($this->session->data['cc_amount']);
			unset( $this->session->data['cc_status']);
			...
			{		
			    ...
			    $data['tracking_id']= $tracking_id;	
		     	$data['amount_done']=$amount;	
   
                $this->session->data['cc_tracking_id'] = $tracking_id;
                $this->session->data['cc_amount'] = $amount;
                $this->session->data['cc_status'] = 1;
                


				$data['text_payment_wait'] = sprintf($this->language->get('text_payment_wait'), $this->url->link('checkout/success_ccavenue'));
				$data['continue'] = $this->url->link('checkout/success');
				$this->response->redirect($this->url->link('checkout/success_ccavenue'));
			...
			{
				$data['text_payment_wait'] = sprintf($this->language->get('text_payment_wait'), $this->url->link('checkout/failure'));
