...
class ControllerPaymentRazorpayredeem extends Controller
{
    public function index()
    {
        ...
        $order_info = $this->model_checkout_order->getOrder_redeem($this->session->data['order_id']);
        ...
        $data['return_url'] = $this->url->link('payment/razorpayredeem/callback', '', 'SSL');
    ...
    public function callback()
    {
        ...
        if (isset($this->request->request['razorpay_payment_id']) and isset($this->request->request['merchant_order_id'])) {
            ...
            $order_info = $this->model_checkout_order->getOrder_redeem($merchant_order_id);
            ...
            if ($success === true) {
                if (!$order_info['order_status_id']) {
                    $this->model_checkout_order->addRedeemOrderHistory($merchant_order_id, $this->config->get('razorpay_order_status_id'), 'Payment Successful. Razorpay Payment Id:'.$razorpay_payment_id);
                } else {
                    $this->model_checkout_order->addRedeemOrderHistory($merchant_order_id, $this->config->get('razorpay_order_status_id'), 'Payment Successful. Razorpay Payment Id:'.$razorpay_payment_id);
                }   
                ...
                echo '<html>'."\n";
                echo '<head>'."\n";
                echo '  <meta http-equiv="Refresh" content="0; url='.$this->url->link('checkout/success_redeem').'">'."\n";
                echo '</head>'."\n";
                echo '<body>'."\n";
