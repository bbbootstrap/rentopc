...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$tax = round($result['price'] *(0.18),0);
			$totalamt = $result['price'] + $tax;
...
$data['entry_bundled'] = $this->language->get('entry_bundled');
		...
		if (isset($this->request->post['product_faqs'])) {
			$product_faqs = $this->request->post['product_faqs'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_faqs = $this->model_catalog_product->getProductFaqs($this->request->get['product_id']);
			//$product_faqs = $this->model_catalog_product->getProductFaqs(40);

		} else {
			$product_faqs = array();
		}

		$data['product_faqs'] = array();

		foreach ($product_faqs as $product_faq) {
			$faq_info = $this->model_catalog_faqs->getfaq($product_faq['id']);

			if ($faq_info) {
				$data['product_faqs'][] = array(
					'faq_id'       => $product_faq['id'],
					'faq_question' => $product_faq['faq_question'],
					'faq_answer' => $product_faq['faq_answer']
				);
			}
		}
		...
		$data['product_relateds'] = array();
			...
			if (isset($this->request->post['product_bundled'])) {
				$product_bundled = $this->request->post['product_bundled'];
			} elseif (isset($this->request->get['product_id'])) {
				$product_bundled = $this->model_catalog_product->getProductBundle($this->request->get['product_id']);
			} else {
				$product_bundled = array();
			}

			$data['product_bundled'] = array();

			foreach ($product_bundled as $bundle) {
				$related_info = $this->model_catalog_product->getProduct($bundle['product_id']);
     ...
     // Bundled product 2 start

		 	if (isset($this->request->post['product_bundled_2'])) {
				$product_bundled_2 = $this->request->post['product_bundled_2'];
			 }
			  elseif (isset($this->request->get['product_id'])) {
					$product_bundled_2 = $this->model_catalog_product->getProductBundle_2($this->request->get['product_id']);
				 } else {
					 $product_bundled_2 = array();
				  }
					 $data['product_bundled_2'] = array();
					 foreach ($product_bundled_2 as $bundle) {
						 $related_info = $this->model_catalog_product->getProduct($bundle['product_id']);
						  if ($related_info) {
								 $data['product_bundled_2'][] = array( 'product_id' => $related_info['product_id'], 'name' => $related_info['name'], 'qty'=>$bundle['qty'] );
							 }
						 }



		 // Bundled product 2 end
