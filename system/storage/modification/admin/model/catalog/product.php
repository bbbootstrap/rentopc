...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
        ...
        if (isset($data['product_faqs'])) {
			foreach ($data['product_faqs'] as $product_faqs) {

				//$this->db->query("INSERT INTO " . DB_PREFIX . "product_faqs SET product_id = '" . (int)$product_id . "', faq_question = '" . (int)$product_faqs['faq_question'] . "', faq_answer = '" .  $this->db->escape($product_faqs['faq_answer']) . "'");
				...
				// if ($product_faqs['faq_id']) {
				// 	foreach ($product_faqs['product_attribute_description'] as $language_id => $product_attribute_description) {
				// 		$this->db->query("INSERT INTO " . DB_PREFIX . "product_attribute SET product_id = '" . (int)$product_id . "', attribute_id = '" . (int)$product_attribute['attribute_id'] . "', language_id = '" . (int)$language_id . "', text = '" .  $this->db->escape($product_attribute_description['text']) . "'");
				// 	}
				// }
			}
		...
		if (isset($data['product_filter'])) {
			...
			$this->db->query("DELETE FROM " . DB_PREFIX . "product_bundle_2 WHERE product_id = '" . (int)$product_id . "'");
			 if (isset($data['product_bundled_2'])) {
    ...
    public function getProductFaqs($product_id) {
		 $product_faq_data = array();

		$product_faq_query = $this->db->query("SELECT * FROM oc_product_faqs WHERE product_id = '" . (int)$product_id . "'");

		foreach ($product_faq_query->rows as $product_faq) {
	...
	public function getProductLayouts($product_id) {
		...
		return $product_layout_data;
			...
			public function getProductBundle($product_id) {
				...
				foreach ($query->rows as $result) {
					$product_related_data[] = array('product_id'=>$result['bundled_id'],'qty'=>$result['qty']);
				}
			...
			public function getProductBundle_2($product_id) {
				 $product_related_data = array();
				 $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_bundle_2 WHERE product_id = '" . (int)$product_id . "'");
				  foreach ($query->rows as $result) {
						 $product_related_data[] = array('product_id'=>$result['bundled_id'],'qty'=>$result['qty']);
					 }
					 return $product_related_data;
